...
class DataLoader_MS2(data.Dataset):
    """A data loader where the glob are arranged in this way:
    ...
    def __init__(self, root, seed=None, data_split='train', modality='thr', 
        ...
        # Ensure seed is an integer before passing to np.random.seed()
        if seed is not None:
            try:
                seed = int(seed)  # Ensure the seed is an integer
            except ValueError:
                raise ValueError(f"Invalid seed value: {seed}. It must be an integer.")
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.modality, list):
            ...
            print(f"RGB data retrieved for index {index}")
            if "nir" in self.modality: results["nir"] = self.data_getter(index, 'nir')
            print(f"NIR data retrieved for index {index}")
            if "thr" in self.modality: results["thr"] = self.data_getter(index, 'thr')
            print(f"THR data retrieved for index {index}")
            results["extrinsics"] = self.get_extrinsic()
            print(f"Extrinsics retrieved for index {index}")
            return results
        else:
    ...
    #     print(f"Attempting to load calib from: {calib_path}") 
    ...
    def set_extrinsics(self):
        # Extrinsics matrices are all same across the sequences, thus use the same values
        calib_path = osp.join(self.root, "sync_data", self.seq_list[0], "calib.npy")
        print(f"Expected calib file path: {calib_path}")
        print(f"File exists: {os.path.isfile(calib_path)}")

        if not os.path.isfile(calib_path):
            raise FileNotFoundError(f"Calib file not found at: {calib_path}")
    ...
    def crawl_folders_depth(self, sampling_step, set_length, set_interval):
        ...
        for sensor in sensor_list:               
            ...
            for seq in self.seq_list: # iterate over each sequence
                ...
                print(f"Left path: {(self.root/'sync_data'/seq/'sensor'/'img_left')}")
                img_list_left      = sorted((self.root/"sync_data"/seq/sensor/"img_left").glob('*.png')) 
                img_list_right     = sorted((self.root/"sync_data"/seq/sensor/"img_right").glob('*.png')) 
    ...
    def get_data_StereoMatching(self, index, modality):
        ...
        result = {}
        result["tgt_image"]      = tgt_img_left  # Add tgt_image as left image for consistency
        result["tgt_left"]       = tgt_img_left
        result["tgt_right"]      = tgt_img_right
        result["tgt_depth_gt"]   = tgt_depth_gt
        result["tgt_disp_gt"]    = torch.as_tensor(
            self.depth2disp(
                tgt_depth_gt,
    ...
    def get_data_Odometry(self, index, modality):
        ...
        pose    = np.stack([
            np.genfromtxt(pose).astype(np.float64).reshape(4,4)[:3,:]
        ...
        result = {}
        result["tgt_image"] = tgt_imgs[0]  # Assign first image in sequence as tgt_image
        result["image_sequence"] = tgt_imgs  # Rename image sequence
        result["pose"] = torch.as_tensor(pose)
        return result